...
async function translateText(text, sourceLang, targetLang, styles, model = "gemini-1.5-flash", styleMeta = {}, candidateCount = 3, options = {}) {
  ...
  if (provider === "gemini" || model.startsWith("gemini")) {
    ...
    const groq = getGroqClient(options.apiKeys?.groq?.trim());
    response = await callOpenAICompatible(groq, model, userPrompt, systemPrompt, {
      signal: options.signal,
      jsonMode
    });
  } else if (provider === "cerebras") {
    const cerebras = getCerebrasClient(options.apiKeys?.cerebras?.trim());
    try {
