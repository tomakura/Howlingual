...
function resolveGroqApiKey(apiKey) {
  return apiKey || ENV.VITE_GROQ_API_KEY;
}
function resolveCerebrasApiKey(apiKey) {
...
async function streamCerebras(modelName, prompt, onUpdate, systemPrompt, apiKey, signal, _jsonMode = true) {
  const resolvedKey = resolveCerebrasApiKey(apiKey);
  await streamProviderChatCompletion({
    url: `${CEREBRAS_BASE_URL}/chat/completions`,
    apiKey: resolvedKey,
